...
## v1.1.4（2026-06-24）— UI 精修与全站文案完善

在 v1.1.0 全新前端基础上，对照 Figma 设计对首页、模型详情、操练场、控制台等模块做了一轮细节打磨，并完善全站交互文案与多语言。

### 首页
- Hero 与「快速开始」「合作伙伴」背景按 Figma 重构；登录后顶部直接显示控制台同款用户组件（不再显示登录按钮）。
- 各带底图 section 横向铺满屏幕（full-bleed），内容仍收进居中列（对齐设计 1440 栅格）。
- Hero 超宽屏插画固定设计尺寸居中、两侧同色无缝填充，不再被放大。
- 合作伙伴轨道图改用矢量 `partners-orbit.svg`（替换原 PNG）。

### 模型广场 / 模型详情
- 模型详情页头部、标签、操作按钮按 Figma 美化；「基本信息 / API 端点 / 分组定价」改用设计风格展示。
- 信息区同时展示**输入价格与输出价格**。

### 操练场（Playground）
- 移除「系统提示 未设」冗余提示，修复思考过程重复展示。
- 消息角色标签改为**图标 + 时间**（去掉「用户 / AI」文字）。

### 控制台
- 侧栏收起态：选中项背景为居中圆角方块，图标水平 + 垂直居中。
- 侧栏收起按钮统一为蓝底箭头小图标，去掉「收起侧边栏」文字。
- 个人设置：「其他设置」标题并入卡片，操作按钮统一宽度，移除底部版权文案。
- 个人设置 / 钱包管理对齐「任务日志」布局（标题 + 副标题、上边距、左右边距与令牌管理一致）。

### 全站
- 操练场、模型广场、表格（API Key / 使用日志 / 任务日志 / 用户等）、认证页、系统设置、关于页等多处样式与交互优化。
- 完善简体中文 / 繁体中文 / 英文文案。
